...
def load_model():
    try:
        ...
        candidate = os.path.join(script_dir, 'models', 'npk_crop_model.pkl')
        candidate_parent = os.path.normpath(os.path.join(script_dir, '..', 'models', 'npk_crop_model.pkl'))

        for p in [candidate, candidate_parent]:
            if os.path.exists(p):
                return joblib.load(p)

        raise FileNotFoundError(f"Model file not found. Tried: {[candidate, candidate_parent]}")
